...
public class SecurityTest implements TestData {
   ...
   * securing one method causing a security vulnerability. Feel free to remove / disable / adapt if
   * you do not use Method Security (e.g. if you prefer Web Security to define who may perform which
   * actions) or want to use Method Security on the service layer.
   */
  @Test
  public void ensureSecurityAnnotationPresentForEveryEndpoint() {
    List<ImmutablePair<Class<?>, Method>> notSecured = components.stream()
        ...
        .flatMap(clazz -> Arrays.stream(clazz.getDeclaredMethods()).map(
            method -> new ImmutablePair<Class<?>, Method>(clazz,
